...
<div style="padding:10px 10px 10px 10px">
	<form id="personSearchForm" class="personForm" method="post">
	    <table cellpadding="5">
	         <tr>
	            <td>身份证号:</td>
	            <td><input id="personId" class="easyui-textbox" type="text" name="personId" data-options="required:true" style="width: 150px;"></input></td>
	            <td>姓名:</td>
	...
	<div style="padding:5px">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="quitForm()">退出</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitForm()">修改</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="deleteForm()">删除</a>
	</div>
</div>
<script type="text/javascript">
	...
	function submitForm(){
		if(!$('#personSearchForm').form('validate')){
			$.messager.alert('提示','信息填写不完整!');
			return ;
		}
		$.post("${pageContext.request.contextPath}/person/update",$("#personSearchForm").serialize(), function(data){
			if(data.status == 200){
	...
	function deleteForm(){
		...
		$.messager.confirm('确认', '确定删除此员工吗？', function(r) {
			if (r) {
				var params = {
					"ids" : personId
				};
				$.post("${pageContext.request.contextPath}/person/delete",params, function(data){
					if(data.status == 200){
						$.messager.alert('提示','删除员工个人信息成功!','info',function(){
							$("#personSearchWindow").window('close');
							$("#personList").datagrid("reload");
						});
					}
