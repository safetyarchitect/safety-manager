...
<div style="padding:10px 10px 10px 10px">
	<form id="inspectionEditForm" class="inspectionForm">
	    <table cellpadding="5">
	          <tr>
	            <td>项目编号:</td>
...
<script type="text/javascript">
	function submitForm(){
		if(!$('#inspectionEditForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return ;
		}
		$.post("${pageContext.request.contextPath}/inspection/update",$("#inspectionEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','修改员工个人信息成功!','info',function(){
					$("#inspectionEditWindow").window('close');
